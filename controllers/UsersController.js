...
const generatePassword = require('../helpers/generatePassword').generatePassword;
...
exports.registerUser = async (req, res) => {
    ...
    try{
        ...
        const addNewUser = new UserModel({ name: req.body.name, email: req.body.email, role: userRoles._id, username: req.body.username, password: hash, emailConfirmation: { token: 'Bearer ' + jwtToken, tokenExparation: new Date(getTokenEXP(jwtToken)),confirmed: false } });
        await addNewUser.save();
        sendActivationEmail(addNewUser, `Bearer ${jwtToken}`);
        return res.json({ message: `Please ${addNewUser.username} go to your Email and confirm your email address in order to log in.`, addNewUser});
    } catch(err) {
...
exports.loginUser = async (req, res) => {
    ...
    try {
        ...
        const payload = { id: user._id, name: user.name, username: user.username, email: user.email, isAdmin: role.isAdmin, role, profilePicture: user.profilePicture, rememberMe: req.body.rememberMe }
...
exports.updateProfilePicture = async (req, res) => {
    try{
        ...
        return res.json({ successMessage: 'Profile Picture is successfuly updated !' });
    }catch (err) {
        return res.json({ failedMessage: err.message })
    }
}
...
exports.getProfilePicture = async (req, res) => {
    try{
        const user = await UserModel.findOne({ _id: new ObjectId(req.query.id) }).select('profilePicture -_id');
...
exports.updateUser = async (req, res) => {
    ...
    try{
        const userId = req.body.id, roleId = req.body.role, userInfo = Object.keys(req.body.userInfo).length > 0 ? req.body.userInfo : null;
        ...
        if(findByNameOrMail && (findUser.email !== findByNameOrMail.email) && (findUser.email !== req.body.email)) { errors.errors.email = "Email already taken"; }
        if(findByNameOrMail && (findUser.username !== findByNameOrMail.username) && (findUser.username !== req.body.username)) { errors.errors.username = "Username already taken"; }
        if(Object.keys(errors.errors).length > 0) { return res.json({ failedMessage: {...errors.errors} }); }
        const emailConfirmation = req.body.confirmed ? {token: "", tokenExparation: null, confirmed: true} : {...findUser.emailConfirmation};
        const user = await UserModel.updateOne({ _id: ObjectId(userId) }, { role: ObjectId(roleId), userInfo, emailConfirmation, name: req.body.name, username: req.body.username, email: req.body.email });
        if(user.nModified === 0) { return res.json({ failedMessage: 'Nothing to update !' }); }
...
exports.getSingleUser = async (req, res) => {
    if(req.query.profile !== 'false') { if(req.query.id !== req.query.id2) { return res.json({ error: `Please do not use other id's` }) } }
    try{
...
exports.addNewUser = async (req, res) => {
    ...
    try{
        ...
        const jwtToken = await jwt.sign({email: req.body.email},secret,{ expiresIn: 86400 });
        const addUser = new UserModel({ name: req.body.name, username: req.body.username, email: req.body.email, userInfo: { facebook: req.body.facebook, instagram: req.body.instagram, twitter: req.body.twitter, github: req.body.github, salary: req.body.salary,telephone: req.body.telephone, country: req.body.country, address: req.body.address, city: req.body.city, postal: req.body.postal, dob: req.body.dob, doe: req.body.doe, }, role: ObjectId(req.body.role), password: hash, emailConfirmation: { token: 'Bearer ' + jwtToken, tokenExparation: new Date(getTokenEXP(jwtToken)), confirmed: false } });
        await addUser.save();
