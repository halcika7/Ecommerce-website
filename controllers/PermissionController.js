...
exports.addPermission = async (req, res) => {
  try {
    ...
    const slug = req.body.permission
      .toLowerCase()
      .split(" ")
      .join("_");
...
exports.deletePermission = async (req, res) => {
  try {
    ...
    await UserRoleModel.updateMany(
      {},
      { $pull: { permissions: req.query.permission } }
    );
...
exports.deleteManyPermissions = async (req, res) => {
  const permissions = Object.keys(req.query).map(
    permission => req.query[permission]
  );
  try {
    const deletedPermissions = await PermissionModel.deleteOne({
