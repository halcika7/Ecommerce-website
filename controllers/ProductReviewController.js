...
exports.addReview = async (req, res, next, socket) => {
	...
	try {
        if(!userId) { return res.json({ failedMessage: 'You are not logged in' });}
        const ifExists = await ProductReviewModel.findOne({ productId, userId: new ObjectId(userId) });
        if(ifExists) { return res.json({ failedMessage: 'You already added review!' });}

		const addReview = new ProductReviewModel({ productId, userId: new ObjectId(userId), rating, text });
        ...
        const findReview = await ProductReviewModel.findOne({ userId: new ObjectId(userId), productId })
...
exports.addReply = async (req, res, next, socket) => {
    ...
    try {
        if(!userId) { return res.json({ failedMessage: 'You are not logged in' });}
        const addReview = new ProductReviewModel({ text, userId: new ObjectId(userId) });

        await addReview.save();

        await ProductReviewModel.updateOne({ _id: new ObjectId(reviewId) }, {
            $push: { replys: { reviewId: new ObjectId(addReview._id), userId: new ObjectId(userId)} }
        });

        const reply = await ProductReviewModel.findOne({ _id: new ObjectId(addReview._id) })
            .populate({path: 'userId', populate: { path: 'userId' }})
        ...
        socket.broadcast.emit('review', {action: 'reply', review: {findReview, reviewId}} );
...
exports.deleteReply = async (req, res, next, socket) => {
    ...
    try {
        ...
        const findParent = await ProductReviewModel.findOne({ 'replys.reviewId':  ObjectId(id), 'replys.userId': ObjectId(userId) })

        const updateReview = await ProductReviewModel.updateOne({ 'replys.reviewId':  ObjectId(id), 'replys.userId': ObjectId(userId) }, 
        ...
        await ProductReviewModel.deleteOne({ _id: ObjectId(id), userId: ObjectId(userId) })

        socket.broadcast.emit('review', {action: 'deleteReply', ids: { parentId: findParent._id , id}} );
...
exports.editReview = async (req,res,next,socket) => {
    try {
        ...
        const findReview = await ProductReviewModel.findOne({ _id: new ObjectId(id) })
            .populate({path: 'userId', populate: { path: 'userId' }})
            .populate({path: 'replys.reviewId', populate: { path: 'reviewId'}})
            .populate({path: 'replys.userId', populate: { path: 'userId'}})

        socket.broadcast.emit('review', {action: 'editReview', review: findReview} );
...
exports.editReply = async (req,res,next,socket) => {
    try {
        ...
        const findReview = await ProductReviewModel.findOne({ _id: new ObjectId(reviewId) })
            .populate({path: 'userId', populate: { path: 'userId' }})
            .populate({path: 'replys.reviewId', populate: { path: 'reviewId'}})
            .populate({path: 'replys.userId', populate: { path: 'userId'}})

        socket.broadcast.emit('review', {action: 'editReply', review: findReview} );
