...
exports.addCategory = async (req, res) => {
    try{
        if(req.body.subcategories.length < 1) { return res.json({ error: 'At least one subcategory required!' }); }
...
exports.editCategory = async (req, res) => {
    try {
        if(req.body.data.subcategories.length < 1){ return res.json({ error: 'At least one subcategory required!' }); }
        await CategoryModel.updateOne({ _id: new ObjectId(req.body.id) }, { ...req.body.data });
        return res.json({ successMessage: 'Category Added !' });
    }catch (err) {
        if(err.errmsg) return res.json({ error: err.errmsg });
        return res.json({ failedMessage: err.message });
...
exports.deleteCategory = async (req, res) => {
    try {
        ...
        if(deletedCategory.n !== 1) { return res.json({ failedMessage: 'Something happened and category was not deleted' }); }
