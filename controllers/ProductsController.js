...
exports.addProduct = async (req, res) => {
	...
	try {
		const name = req.body.name, price = parseInt(req.body.price), year = parseInt(req.body.year), brand = req.body.brand, category = req.body.category, optionsDiscount = parseInt(req.body.optionsDiscount), published = JSON.parse(req.body.published), featured = JSON.parse(req.body.featured), dailyOffer = JSON.parse(req.body.dailyOffer), weeklyOffer = JSON.parse(req.body.weeklyOffer), description = req.body.description, smalldescription = req.body.smalldescription.replace(/<\/?[^>]+(>|$)/g,''), bluetooth = req.body.bluetooth, wifi = req.body.wifi, subcategories = JSON.parse(req.body.subcategories), rawOptions = JSON.parse(req.body.options),
		...
		await req.files.forEach(
			async file =>
				await fs.move(file.path,`public/images/products/${req.body.name}/${file.filename}`)
		);

		const options = rawOptions.map(opt => {
			const newOpt = { ...opt };
			if (dailyOffer || weeklyOffer) { newOpt.options = newOpt.options.map(opt => ({...opt,discount: optionsDiscount}));}
			const findFeaturedPictureInFiles = files.find(file => file.originalname === opt.featuredPicture);
			if (findFeaturedPictureInFiles) { newOpt.featuredPicture = findFeaturedPictureInFiles.path; }
			newOpt.pictures = opt.pictures.map(picture => {
				const findPicture = files.find(file => file.originalname === picture);
				if (findPicture) {return findPicture.path;}
			});
			return newOpt;
		});

		const addProduct = new ProductModel({ name, price, year, brand, category, published, featured, description, smalldescription, wifi, bluetooth, subcategories, options,
			weeklyOffer: weeklyOffer ? offer : null, dailyOffer: dailyOffer ? offer : null });
...
exports.getBannerProducts = async (req, res) => {
	try{
		const bannerProducts = await ProductModel.aggregate([
			{$match: {published: true,featured: true,dailyOffer: null,weeklyOffer: null}},
			{ $sort: { createdAt: -1 } },
			{ $sample: { size: 4 } }
		]);
...
exports.getProduct = async (req, res) => {
	try {
		...
		if (!product) {return res.json({ failedMessage: 'Product not found with provided ID' });}
...
exports.searchForProduct = async ( req, res ) => { 
	...
	try {
		const products = await ProductModel.aggregate( [
			...
			{ $limit: 12 },
			{ $project: { name: 1, price: 1, category: 1, brand: 1, 'options.featuredPicture': 1, 'first': { $arrayElemAt: ["$options.featuredPicture", 0] }, "sizeOfArray": { $size: "$options" }}}
		] );
...
exports.getAllProducts = async (req, res) => {
	try {
		const products = await ProductModel.aggregate([
			{$match: {}},
			{$project: { name: 1, prodRating: '$rating.averageRating',price: 1, category: 1, brand: 1, year: 1, numberOfsales: 1, published: 1, featured: 1, 'featuredPicture': { $arrayElemAt: ["$options.featuredPicture", 0] },
				numberOfOptions: { 
					$cond: { 
						if: { $isArray: "$options" }, 
						then: { $size: "$options" }, 
...
exports.deleteProduct = async (req, res) => {
	try {
		...
		const dd = await ProductReviewModel.aggregate([
            { $match: { 'replys.reviewId': { $exists: true }, productId: req.query.id } },
