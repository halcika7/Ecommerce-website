...
module.exports = addUserValidation = async data => {
	...
	if (Validator.isEmpty(userData.address))
		errors.address = 'Address field is required';
	if (Validator.isEmpty(userData.telephone))
		errors.telephone = 'Telephone field is required';
	if (Validator.isEmpty(userData.country))
		errors.country = 'Country field is required';
	if (Validator.isEmpty(userData.city)) errors.city = 'City field is required';
	if (Validator.isEmpty(userData.zip))
		errors.zip = 'Zip/postal field is required';
	...
	if (items.length > 0) {
		const promises = await items.map(async item => {
			const findSkuOption = await ProductModel.aggregate([
				{
					$match: {
						...
						'options.options.quantity': { $gte: item.quantity }
					}
				},
				{ $unwind: '$options' },
			...
			if (findSkuOption.length !== 1) {
				return {
					error: `Product with sku=${item.sku} not available with quantity=${
						item.quantity
					}`
				};
			}
	...
	if (Object.keys(coupon).length > 0) {
		try {
			...
			if (!checkCoupon) {
				errors.coupon = 'Provided coupon code expired';
			}
		} catch (err) {
	...
	try {
		...
		if (response.verifications.delivery.success === true) {
			...
			if (userData.zip !== response.zip) {
				errors.zip = `Zip/postal code does not match. Maybe you thought ${
					response.zip
				}`;
			}
		}
		...
		if (
			err.error &&
			err.error.error &&
			err.error.error.code === 'ADDRESS.PARAMETERS.INVALID'
		) {
