		...
		it('should throw an error if accessing the database fails', done => {
			...
			server
				...
				.end((err, res) => {
					...
					res.body.errors.usernameEmail.should.equal(
		...
		it('should throw an error if accessing the database fails', done => {
			...
			server
				...
				.end((err, res) => {
					...
					res.body.errors.password.should.equal(
						'Password must be at least 6 characters'
					);
					res.body.errors.password2.should.equal(
	...
	describe('Reset Password', () => {
		...
		it('should throw an error if provided password is already in use', done => {
			...
			server
				...
				.end((err, res) => {
					res.status.should.equal(200);
					res.body.failedMessage.should.equal(
						'Provided password is already in use'
					);
					done();
				});
		});
		...
		it('should throw an error if provided file path equals to user.pprofilePicture', done => {
			...
			server
				...
				.end((err, res) => {
					res.status.should.equal(200);
					res.body.failedMessage.should.equal(
						'Selected picture is same as the one u already use !'
					);
					done();
				});
		});
	...
	describe('Get User', () => {
		...
		it('should thorw an error if user not found', done => {
			server
				...
				.query({
					profile: true,
					id: '5c7fbe70ef00877b015fc1b5',
					id2: '5c7fbe70ef00877b015fc1b5'
				})
				.expect('Content-type', /json/)
				.end((err, res) => {
					res.status.should.equal(200);
