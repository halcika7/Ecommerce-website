...
describe('User Roles Controller', () => {
	describe('Add User Role', () => {
		it('should throw an error if role name is empty', done => {
			const req = { name: '' };
			server
				.post('/rolespermissions/roles/adduserrole')
				.send(req)
			...
			server
				...
				.end((err, res) => {
					...
					res.body.failedMessage.should.equal(
						'E11000 duplicate key error collection: onlineShop.roles index: name_1 dup key: { : "Owner" }'
					);
					done();
				});
		});
	...
	describe('Get Roles', () => {
		it('respond with json containing a list of all roles', done => {
			server
				.get('/rolespermissions/roles/userroles')
