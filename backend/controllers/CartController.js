...
exports.addToCart = async (req, res) => {
  const { sku, cartItems: cartItms } = req.query;
  const token = cartItms !== 'null' ? cartItms.split(' ')[1] : '';
  ...
  if (findIndex !== -1)
    return res.json({ failedMessage: 'Item already in cart' });
  ...
  try {
    ...
    const findSkuOption = await ProductModel.aggregate([
      {
        $match: {
          'options.options.sku': sku,
          'options.options.quantity': { $gt: 0 }
        }
      },
      { $unwind: '$options' },
    ...
    product.total =
      product.discount > 0
        ? product.price +
          product.aditionalPrice -
          (product.price + product.aditionalPrice) * (product.discount / 100)
        : product.price + product.aditionalPrice;
    product.quantity = 1;
    ...
    const subtotalBefore = +(
      Math.round(cartItems.reduce((acc, curr) => acc + curr.total, 0) + 'e+2') +
      'e-2'
    );
    const cartSubtotal =
      checkCoupon && checkCoupon.type === 'percent'
        ? cartItems.reduce((acc, curr) => acc + curr.total, 0) -
          cartItems.reduce((acc, curr) => acc + curr.total, 0) *
            (checkCoupon.value / 100)
        : checkCoupon && checkCoupon.type === 'value'
        ? cartItems.reduce((acc, curr) => acc + curr.total, 0) -
          checkCoupon.value
        : cartItems.reduce((acc, curr) => acc + curr.total, 0);
    ...
    const cartPrice = {
      subtotalBefore,
      subtotal: +(Math.round(cartSubtotal + 'e+2') + 'e-2'),
      total: cartTotal,
      tax
    };

    const jwtToken = await jwt.sign(
      { items: cartItems, cartPrice, coupon },
      secret,
      null
    );
...
exports.deleteFromCart = async (req, res) => {
  try {
    ...
    const token =
      req.query.cartItems !== 'null' ? req.query.cartItems.split(' ')[1] : '';
    const decoded = jwt.decode(token);
    const cartItems = decoded ? decoded.items : [];
    const findIndex = cartItems.findIndex(item => item.sku === sku);

    if (findIndex === -1)
      return res.json({ failedMessage: 'Item not in cart' });
    ...
    const checkCoupon = await checkCouponObj(couponQuery);
    const coupon =
      checkCoupon && cartItems.length > 0 ? { ...checkCoupon } : {};
...
exports.updateCartItem = async (req, res) => {
  ...
  if (value < 1) {
    return res.json({ failedMessage: 'Invalid value entered' });
  } else if (findIndex === -1) {
    return res.json({ failedMessage: 'Item not in cart' });
  }
  ...
  try {
    const findSkuOption = await ProductModel.aggregate([
      {
        $match: {
          'options.options.sku': sku,
          'options.options.quantity': { $gte: value }
        }
      }
    ]);

    if (findSkuOption.length === 0) {
      return res.json({
        failedMessage: 'That quantity is not available for that product'
      });
    }
        ...
        newItem.total =
          newItem.discount > 0
            ? (newItem.price + newItem.aditionalPrice) * value -
              (newItem.price + newItem.aditionalPrice) *
                value *
    ...
    const subtotalBefore = +(
      Math.round(
        updatedCartItems.reduce((acc, curr) => acc + curr.total, 0) + 'e+2'
      ) + 'e-2'
    );
    const cartSubtotal =
      Object.keys(coupon).length > 0 && coupon.type === 'percent'
        ? updatedCartItems.reduce((acc, curr) => acc + curr.total, 0) -
          updatedCartItems.reduce((acc, curr) => acc + curr.total, 0) *
            (checkCoupon.value / 100)
        : Object.keys(coupon).length > 0 && coupon.type === 'value'
        ? updatedCartItems.reduce((acc, curr) => acc + curr.total, 0) -
          checkCoupon.value
        : updatedCartItems.reduce((acc, curr) => acc + curr.total, 0);
    const tax = +(Math.round(cartSubtotal * 0.17 + 'e+2') + 'e-2');
    ...
    const jwtToken = await jwt.sign(
      { items: updatedCartItems, cartPrice, coupon },
      secret,
      null
    );
...
exports.moveToSaveForLater = async (req, res) => {
  ...
  const token =
    req.query.cartItems !== 'null' ? req.query.cartItems.split(' ')[1] : '';
  const tokenSave =
    req.query.saveForLaterItems !== 'null'
      ? req.query.saveForLaterItems.split(' ')[1]
      : '';
  ...
  const savedForLaterItems = decodedSaveForLaterItems
    ? decodedSaveForLaterItems.savedForLater
    : [];
  const findIndex = cartItems.findIndex(item => item.sku === sku);
  ...
  if (findIndex === -1) {
    return res.json({ failedMessage: 'Item not in Cart' });
  } else if (findIndexSave !== -1) {
    return res.json({ failedMessage: 'Item already Saved for later' });
  }

  try {
    const newSavedProduct = {
      ...cartItems[findIndex],
      total: cartItems[findIndex].total / cartItems[findIndex].quantity,
      quantity: 1
    };
    cartItems.splice(findIndex, 1);
    ...
    const jwtTokenSave = await jwt.sign(
      { savedForLater: savedForLaterItems },
      secret,
      null
    );
    ...
    return res.json({
      token: 'Bearer ' + jwtToken,
      save: 'Bearer ' + jwtTokenSave,
      successMessage: 'Item moved to save for later'
    });
  } catch (err) {
    console.log(err);
    if (err.errmsg) return res.json({ failedMessage: err.errmsg });
...
exports.moveToCart = async (req, res) => {
  ...
  if (findIndex !== -1) {
    ...
    return res.json({ failedMessage: 'Item not saved for later' });
  }
...
exports.deleteFromSavedForLater = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      save: 'Bearer ' + jwtTokenSave,
      successMessage: 'Item deleted from saved for later'
    });
  } catch (err) {
    console.log(err);
    if (err.errmsg) return res.json({ failedMessage: err.errmsg });
...
exports.applyCoupon = async (req, res) => {
  ...
  try {
    ...
    if (!checkCoupon) {
      return res.json({ failedMessage: 'Invalid coupon!' });
    }
