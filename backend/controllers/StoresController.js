...
exports.addStore = async (req, res) => {
  ...
  try {
    const addStore = new StoresModel({ ...storeData });
    await addStore.save();
...
exports.updateStore = async (req, res) => {
  ...
  const { errors, isValid } = await addStoreValidation(req.body, req.file, id);
...
exports.deleteStore = async (req, res) => {
  try {
    const { id } = req.query;
    const findStore = await StoresModel.findOne({
      _id: new ObjectId(id)
    });

    if (findStore) await fs.remove(findStore.picture);

    await StoresModel.deleteOne({ _id: new ObjectId(id) });
