...
export const register = (UserObj) => async dispatch => {
        ...
        if(response.data.failedMessage) {
            ...
            dispatch({ type: actionTypes.REGISTER_SUCCESS, User: response.data.addNewUser, message: response.data.message });
        }
}
...
export const activateAccount = (activationObject) => async dispatch => {
    ...
    if(response.data.failedMessage) {
        ...
        dispatch({ type: actionTypes.REGISTERACTIVATION_SUCCESS, successMessage: response.data.successMessage });
        setTimeout(() => { dispatch({type: actionTypes.REGISTERACTIVATION_CLEAR }); }, 8000);
    }
}
...
export const resendActivationLink = activationObject => async dispatch => {
    ...
    if(response.data.failedMessage) {
        ...
        dispatch({ type: actionTypes.REGISTERACTIVATION_SUCCESS, successMessage: response.data.successMessage });
        setTimeout(() => { dispatch({type: actionTypes.REGISTERACTIVATION_CLEAR }); }, 4000);
    }
}