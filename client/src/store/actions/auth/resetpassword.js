...
export const resetPassword = (passwords) => async dispatch => {
    try {
        ...
        if(response.data.errors){
            dispatch({type: actionTypes.RESETPASSWORD_FAILED, errors: response.data.errors, passwords});
        }
        else {
        ...
        dispatch({type: actionTypes.RESETPASSWORD_FAILED, errors: err.response.data, passwords});
    }
}
