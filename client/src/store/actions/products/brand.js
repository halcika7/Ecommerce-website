...
export const addBrand = brandData => async dispatch => {
    ...
    if(response.data.error) {
        dispatch({ type: actionTypes.BRAND_FAILED, error: response.data.error, data: brandData });
    }else if (response.data.failedMessage) {
        dispatch({ type: actionTypes.BRAND_FAILED, failedMessage: response.data.failedMessage, data: brandData });
    }else {
        dispatch({ type: actionTypes.BRAND_SUCCESS, successMessage: response.data.successMessage });
...
export const editBrand = (id, data) => async dispatch => {
    ...
    if(response.data.failedMessage) {
        dispatch({ type: actionTypes.BRAND_FAILED, failedMessage: response.data.failedMessage, data });
    }else {
        dispatch({ type: actionTypes.BRAND_SUCCESS, successMessage: response.data.successMessage });
