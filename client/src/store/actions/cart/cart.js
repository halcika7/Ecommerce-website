...
export const processPayment = (token, userData, callBack) => async dispatch => {
    ...
    const decoded = cart ? jwt_decode(cart) : { cartItems: [], totals: {}, coupon: 0 };
    const response = await axios.post('/cart/checkout/payment', data , { headers: { 'Authorization': jwtToken } });
    ...
    if(response.data.successMessage) { 
        dispatch({ type: actionTypes.CART_SUCCESS, successMessage: response.data.successMessage }); 
