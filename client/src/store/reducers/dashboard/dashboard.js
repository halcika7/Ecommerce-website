...
const reducer = (state = initialState, action) => {
	const actions = {
		...
		[actionTypes.DASHBOARD_SUCCESS]: updateObject(state, {
			...
			numberOfActiveCoupons: action.numberOfActiveCoupons
				? action.numberOfActiveCoupons
				: 0,
			numberOfSoldProducts: action.numberOfSoldProducts
				? action.numberOfSoldProducts
				: 0,
			latestTransactions: action.latestTransactions
				? action.latestTransactions
				: [],
			bestSellingProducts: action.bestSellingProducts
