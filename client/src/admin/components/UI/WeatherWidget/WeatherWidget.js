...
const WeatherWidget = props => {
	...
	const getUserLocation = async () => {
		if (!localStorage.location) {
			const user = await fetch(
				'https://api.ipdata.co/?api-key=83a6049fdd6a9c358a1492180a87d5164e130b0162faca9627a7f782'
			);
			const { longitude, latitude, city, country_name } = await user.json();
			setPositions({ longitude, latitude, city, country_name });
			localStorage.setItem(
				'location',
				JSON.stringify({ longitude, latitude, city, country_name })
			);
		} else {
	...
	const weatherApi = async (long, lat) => {
		...
		const weather = await fetch(
			`${proxy}https://api.darksky.net/forecast/06776b9a1d887f4a3cb4543190b21c6b/${lat},${long}`
		);
		...
		setWeatherData({
			...
			summary,
			day: new Date(time * 1000).toLocaleDateString('en-US', {
				weekday: 'long'
			}),
				...
				<div className="card card-statistics text-center h-100">
					<div className="bg-white pb-15 pt-15">
						...
						<h4 className="mb-0 text-dark">
							{positions.city}, {positions.country_name}
						</h4>
					</div>
					<div className="card-body">
						<div className="weather-icon py-3 text-warning">
							<i className={`wi wi-forecast-io-${weatherData.icon}`} />
						</div>
						...
						<div className="divider mt-30 mb-30" />
								...
								return (
									<div key={index} className="col-6 col-md-3 col-sm-3">
										<div className="weather-icon icon2 mb-20 text-white">
											<i className={`wi wi-forecast-io-${day.icon}`} />
