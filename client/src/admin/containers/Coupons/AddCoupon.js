...
const AddCoupon = props => {
  ...
  const [errors, setErrors] = useState({ name: '', type: '', expiresIn: '', value: '' });
  ...
  return (
    <div className={"AdminProfile row"}>
      {props.coupon.failedMessage && (<ResponseMessage message={props.coupon.failedMessage} ClassName="Danger" /> )}
      {props.coupon.successMessage && (<ResponseMessage message={props.coupon.successMessage} /> )}
        ...
        <div className="Card card text-white">
            ...
            <div className="col-12">
                ...
                <div className="row">
                    ...
                    <LoginRegisterInputs type="text" placeholder="Coupon Code" inputClass="form-control" invalidInput="invalid" invalidFeedback="invalid-feedback" value={couponName} onChange={e => setCouponName(e.target.value)} error={errors.name ? errors.name : ''} />
                    </div>
                    <div className="col-12">
                    <label className="text-white d-block">Coupon Type</label>
                        <select className={errors.type ? "select invalid" : "select"} onChange={e => setCouponType(e.target.value)}>
                    ...
                    <label className="text-white d-block">Expires In</label>
                        <select className={errors.expiresIn ? "select invalid" : "select"} onChange={e => setExpiresIn(e.target.value)}>
                        ...
                        {errors.expiresIn && <div className='invalid-feedback'>{errors.expiresIn}</div>}
                    </div>
                    <div className="col-12 mt-3">
                    <label className="text-white d-block">Value</label>
                        {couponType === 'percent' ? 
                            <input className={errors.value ? "form-control invalid" : "form-control"} name='value' type='number' min='1' max='100' placeholder="Enter percentage value" value={value} onChange={e => setValue(e.target.value)} /> :
                            <input className={errors.value ? "form-control invalid" : "form-control"} name='value' type='number' min='1' placeholder="Enter money value" value={value} onChange={e => setValue(e.target.value)} /> 
                        }
                        {errors.value && <div className={'invalid-feedback'}>{errors.value}</div>}
                    </div>
                </div>
                    <button className="btn btn-sm mt-20" onClick={submitData}>Add New Coupon Code</button>
                </form>
            </div>
            </div>
        </div>
