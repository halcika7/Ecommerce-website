...
const ColorsSizesComponent = props => {
    ...
    useEffect(() => {
        if(props.choosenSizes) {
            console.log(props.choosenSizes)
    ...
    return (
        <div className="mb-20">
            {props.color && <Select options={colors} onChange={e => props.setColor(e.label)} styles={colourStyles} value={ props.value === '' ? {label: 'Choose Color'} : { label: props.value, value: props.value.toLowerCase()} } required/>}
            {props.sizes && <Select options={sizes} onChange={e => props.setSize(e.label)} styles={sizeStyles} value={ props.value === '' ? {label: 'Choose Size'} : { label: props.value } } required/>}
        </div>
    );
}
