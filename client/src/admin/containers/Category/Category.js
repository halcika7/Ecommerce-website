...
const Category = props => {
    ...
    useEffect(() => { if(props.category.errorId) { setTimeout(() => props.history.goBack(), 6000); } }, [props.category.errorId]);
    useEffect(() => { setCatName(props.category.categoryData.name); setIconName(props.category.categoryData.icon); setAllSubCategories(props.category.categoryData.subcategories); }, [props.category.categoryData]);
    useEffect(() => { setCatNameError(props.category.error); }, [props.category.error]);

    const addSubCategory = e => {
        e.preventDefault();
        if(subCatName.length < 5) { setSubCatNameError('Name must be atleast 5 characters !'); return; }
        if(subCategories.length < 1) { setSubCatNameError('At least one subcategory required!'); return; }
        const newAllSubCategories = [...allSubCategories];
        newAllSubCategories.push({ name: subCatName, subcategories: subCategories });
        setAllSubCategories(newAllSubCategories); setSubCatName(''); setSubCategories(''); setSubCatNameError('');
    }

    const addCategory = e => {
        e.preventDefault();
        if(catName.length < 5) { setCatNameError('Name must be atleast 5 characters !'); return; } else { setCatNameError(''); }
        const catData = { name: catName, icon: iconName, subcategories: allSubCategories };
        setCatName(''); setSubCatName(''); setSubCategories([]); setAllSubCategories([]);
        if(props.addcategory){ props.addCategory(catData); }
        if(props.editcategory) { const id = new URLSearchParams(props.location.search).get('id'); props.editCategory(id, catData); }
    }

    const deleteSubCategory = index => { const newAllSubCategories = [...allSubCategories]; newAllSubCategories.splice(index, 1); setAllSubCategories(newAllSubCategories); }

    const deleteTagSubcategory = (subCatIndex, tagIndex) => {
        const newAllSubCategories = [...allSubCategories]; newAllSubCategories[subCatIndex].subcategories.splice(tagIndex, 1);
        if(newAllSubCategories[subCatIndex].subcategories.length < 1) { delete newAllSubCategories[subCatIndex]; }
    ...
    const onInputKeyDown = (e, index) => {
        if(e.key === "Enter") {
            ...
            if(e.target.value.length < 5) { errors[index] = `Name must be at least 5 characters ${index} !`; setInputErrors(errors); return;}
            else { errors.splice(index, 1); setInputErrors(errors); }
            const newAllSubCategories = [...allSubCategories]; newAllSubCategories[index].subcategories.push(e.target.value); setAllSubCategories(newAllSubCategories);
            e.target.value = '';
        }
    }

    const startFunction = () => {
        if(!props.addcategory) { const id = new URLSearchParams(props.location.search).get('id'); props.getCategory(id); props.getAllCategoryIcons(); }
        if(props.addcategory) { props.clearState(); props.getAllCategoryIcons(); }
    }
    ...
    return (
        <div className='AdminProfile row'>
            ...
            <div className='col-12 mb-30'>
                {props.category.loading || props.category.errorId ? <div className="card bg-white"><SmallSpinner /></div> :
                <div className="Card card text-white">
                    ...
                    <div className="card-body">
                        <form>
                            <div className="row">
                                <LoginRegisterInputs formBox="col-md-6" label="Category Name" type="text" name="catName" placeholder="Enter Category Name" inputClass='form-control' invalidInput='invalid' invalidFeedback='invalid-feedback' value={catName}onChange={e => setCatName(e.target.value)} error={catNameError ? catNameError : ''} disabled={props.viewcategory}/>
                            ...
                            {(props.addcategory || props.editcategory) && <div className="row">
                                <LoginRegisterInputs formBox="col-md-6" label=" Sub Category Name" type="text" name="subCatName" placeholder="Enter Sub Category Name" inputClass='form-control' invalidInput='invalid' invalidFeedback='invalid-feedback' value={subCatName} onChange={e => setSubCatName(e.target.value)} error={subCatNameError ? subCatNameError : ''} disabled={props.viewcategory}/>
                                <div className="col-md-6">
