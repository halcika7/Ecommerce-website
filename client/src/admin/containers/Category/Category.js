...
const Category = props => {
  ...
  useEffect(() => {
    startFunction();
  }, [props.addcategory, props.viewcategory, props.editcategory]);
  useEffect(() => {
  ...
  useEffect(() => {
    setCatNameError(props.category.error);
  ...
  return (
    <div className="AdminProfile row">
      {props.category.failedMessage && (
        <ResponseMessages
          message={props.category.failedMessage}
          ClassName="Danger"
        />
      )}
      {props.category.errorId && (
        <ResponseMessages message={props.category.errorId} ClassName="Danger" />
      )}
      {props.category.successMessage && (
        <ResponseMessages message={props.category.successMessage} />
      )}
              ...
              <form>
                <div className="row">
                  <LoginRegisterInputs
                    ...
                    error={catNameError ? catNameError : ''}
                    disabled={props.viewcategory}
                  />
                        ...
                        props.allIcons.map(icon => (
                          <label key={icon._id} className={classes.radioLabel}>
                            <input
                              ...
                              checked={icon.name === iconName ? true : false}
                            />
                ...
                {(props.addcategory || props.editcategory) && (
                  <div className="row">
                    <LoginRegisterInputs
                      formBox="col-md-6"
                      ...
                      error={subCatNameError ? subCatNameError : ''}
                      disabled={props.viewcategory}
                    />
                    <div className="col-md-6">
                      <label className="text-white">Enter Subcategories</label>
                      <TagsInput2
                        choosenValues={subCategories}
                        setChoosenValues={setSubCategories}
                        disabled={props.viewcategory}
                      />
                    </div>
                    {subCatName.length > 4 && subCategories.length > 0 && (
                      <button
                        type="button"
                    ...
                    {allSubCategories.map((cat, catIndex) => (
                      <div className="col-md-6 mt-20" key={catIndex}>
                        <p className={classes.P}>
                          ...
                          {!props.viewcategory && (
                            <i
                              onClick={() => deleteSubCategory(catIndex)}
                          ...
                          {cat.subcategories.map((sub, index) => (
                            <li key={index}>
                              ...
                              {!props.viewcategory && (
                                <i
                                  onClick={() =>
                                    deleteTagSubcategory(catIndex, index)
                                  }
                                  className="fas fa-times"
                                />
                              )}
                        ...
                        {!props.viewcategory && (
                          <React.Fragment>
                            ...
                            <LoginRegisterInputs
                              type="text"
                              name={catIndex}
                              placeholder="Add another subcategory"
                              inputClass="form-control"
                              invalidInput="invalid"
                              invalidFeedback="invalid-feedback"
                              onKeyDown={e => onInputKeyDown(e, catIndex)}
                              error={
                                inputErrors[catIndex]
                                  ? inputErrors[catIndex]
                                  : ''
                              }
                              disabled={props.viewcategory}
...
const dispatchMapToProps = dispatch => {
  return {
    addCategory: (data, callBack) =>
      dispatch(actions.addCategory(data, callBack)),
    getCategory: (id, callBack) => dispatch(actions.getCategory(id, callBack)),
    editCategory: (id, data, callBack) =>
      dispatch(actions.editCategory(id, data, callBack)),
    clearState: () => dispatch(actions.clearState()),
    getAllCategoryIcons: callBack =>
      dispatch(actions.getAllCategoryIcons(callBack))
  };
};
