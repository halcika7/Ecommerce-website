...
const Brand = props => {
    ...
    return (
        <div className={'AdminProfile row'}>
            {props.brand.failedMessage && <ResponseMessages message={props.brand.failedMessage} ClassName="Danger" />}
                ...
                <React.Fragment>
                    <div className="Card card text-white">
                        <div className="card-header">
                            ...
                            {props.viewbrand && <h4 className="text-white">{brandName} Brand</h4>}
                        ...
                        <div className="card-body">
                            <div className="col-12">
                                <form>
                                    <div className="row">
                                        ...
                                        <div className="col-12 mb-20">
                                            ...
                                            <div className={classes.checkBoxWrapper}>
                                                {props.addbrand && props.categories.map(category => 
                                                <div className={classes.checkbox} key={category._id}>
                                                    <input type="checkbox" onChange={(e) => checkBoxOnChange(e, category.name)} />
                                                    <label>{category.name}</label>
                                                </div>)}
                                                {props.viewbrand && selectedCategories.map((category, index) => 
                                                <div className={classes.checkbox} key={index}>
                                                    <input type="checkbox" value={category} defaultChecked={true} disabled={true} />
                                                    <label>{category}</label>
                                                </div>)}
                                                {props.editbrand && props.categories.map(category => {
                                                    const find = selectedCategories.find(cat => category.name === cat);
                                                    return <div className={classes.checkbox} key={category._id}>
                                                        <input type="checkbox" onClick={(e) => checkBoxOnChange(e, category.name)} defaultChecked={find}/>
                                                        <label>{category.name}</label>
                                                    </div>
                                                })}
                                            </div>
                                    ...
                                    {!props.viewbrand && <button className="btn btn-sm mt-20" onClick={submitData}>{props.addbrand ? 'Add New Brand' : 'Update Brand'}</button>}
                                </form>
                            </div>
                        </div>
                    </div>
