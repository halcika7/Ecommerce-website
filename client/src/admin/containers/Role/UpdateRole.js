...
const UpdateRole = props => {
    ...
    useEffect(() => { const id = new URLSearchParams(props.location.search).get('id'); props.getRole(id); props.getPermissions(); }, []);
    useEffect(() => { setRole({ ...props.roleState }); setRoleName(props.roleState.name); setIsAdmin(props.roleState.isAdmin); setChoosenPermissions(props.roleState.permissions); setAllPermissions(props.allPermissions);}, [props.roleState]);
    ...
    const onFormSubmit = e => { e.preventDefault(); const roleData = { id: role._id, name: roleName, isAdmin, permissions: choosenPermissions }; props.updateRole(roleData); }

    if(props.failedMessage === true) { return <Redirect to='/admindashboard/dashboard'/>; }
    ...
    return (
        ...
        {props.errorId ? <ResponseMessages ClassName="Danger" message={props.errorId} /> : null}
            <div className='col-12'>
                {props.errorId || ! roleName ? <div className="card bg-white"><SmallSpinner /></div> : 
                <div className="card mb-30 text-white">
                    <div className="card-header"><h4>Update {props.roleState.name} Role</h4></div>
                    <div className='card-body'>
                        <form onSubmit={onFormSubmit}>
                            <div className='mb-20'>
                                <label className='d-block'>Role Name</label>
                                <input type="text" placeholder="Enter Role Name" value={roleName} onChange={e => setRoleName(e.target.value)} style={{ outline: '0px', boxShadow: 'none', border: '1px solid rgba(255, 255, 255, 0.489)', marginBottom: '0px' }} className={classnames( 'w-100 d-block', {'invalid': props.failedMessage})} disabled={props.view}/>
                            ...
                            <div className="mb-30">
                                ...
                                <TagsInput values={allPermissions} choosenValues={choosenPermissions} setChoosenValues={setChoosenPermissions} disabled={props.view}/>
                            </div>
