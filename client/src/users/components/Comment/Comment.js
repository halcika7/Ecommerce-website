...
const Comment = props => {
    ...
    const onSubmitReply = e => {
        e.preventDefault();
        props.addReply({ text: replyText, userId: props.userId, reviewId: props.review._id });
        setReplyText('');
        setShowReply(false);
    ...
    return (
        <li className="comment">
            <div className="comment-author d-sm-block d-none">
                <img src={props.review.userId.profilePicture} alt={props.review.userId.username} />
            </div>
            <div className="comment-text">
                <div className="top">
                    <span><b>{props.review.userId.username}: </b></span>
                    <button className="like"><i className="fas fa-thumbs-up"></i>{props.review.likes.length}</button>
                    <button className="dislike"><i className="fas fa-thumbs-down"></i>{props.review.dislikes.length}</button>
                ...
                {!editcomment && 
                    <div className="text" dangerouslySetInnerHTML={{ __html: props.review.text }}></div>
                }
                {editcomment && 
                    <textarea className="text" placeholder="Edit Review Comment" value={editedReviewText} onChange={editReviewText}></textarea>
                }
                <span className='time-ago'>
                    <TimeAgo date={props.review.createdAt} />
                </span>
                {showReply && 
                    <textarea className="text" placeholder="Enter Reply Comment" onChange={e => setReplyText(e.target.value)}></textarea>
                }
                {replyText.length > 0 && showReply && 
                    <button type='button' onClick={onSubmitReply} >Submit Reply</button>
                }
                <div className="comment-footer">
                    ...
                    {props.review.userId._id === props.userId && 
                        <React.Fragment>
                            ...
                            {editcomment && 
                                <button type='button' onClick={submitEditedReview}>Submit Edited Review</button>
                            }
                        </React.Fragment>
                    }
