...
const StoreLocator = props => {
    ...
    useEffect(() => {
        if(stores.length > 0) {
            ...
            const markers = stores[0].stores[0].stores.map((stores,j) => ({...stores.location,index:0,i:0,j}));
            setMarkers(markers);
        }
    }, [stores])
    ...
    const showStore = (e,index,i,j) => {
        ...
        const markers = stores[index].stores[i].stores.map((stores,j) => ({...stores.location, index, i, j}));
        setMarkers(markers);
    }
            ...
            <div className={c.containerfluid + " container-fluid " + c.storelocator}>
                <div className={c.row + " row"} style={{padding:0}}>
                        ...
                        <ul className={c.countries}>
                            {stores.map((countries, index) => 
                                <li key={countries._id}>
                                    <a className={c.country} data-toggle="collapse" href={"#"+countries._id} role="button" aria-expanded="false">{countries._id}</a>
                                    <ul className={c.collapse+ " collapse"} id={countries._id}>
                                        {countries.stores.map((city, i) => 
                                            <li key={city._id.city}>
                                                <a className={c.city} data-toggle="collapse" href={"#"+city._id.city} role="button" aria-expanded="false">{city._id.city}</a>
                                                <ul className={c.collapse+ " collapse"} id={city._id.city}>
                                                    {city.stores.map((store, j) => 
                                                        <li key={store._id}>
                                                            <a className={c.city} href="/" onClick={e => showStore(e,index,i,j)}>{store.address}</a>
                                                        </li>
                                                    )}
                                                </ul>
                                            </li>
                    ...
                    <div className={c.colxl2 + " col-xl-2 " + c.storedescription}>
                        ...
                        <p >{store.email}</p>
                        <h6>Opening Hours</h6>
                        <p >Mon-Fri: {store.weekHours.from} - {store.weekHours.to}</p>
                        <p >Saturday: {store.saturdayHours.from} - {store.saturdayHours.to}</p>
                    </div>}
...
export default connect(mapSatateToProps, dispatchMapToProps)(StoreLocator);