...
const LoginForm = props => {
    ...
    const [inputs] = useState([
        {name: 'usernameEmail',placeholder: 'Username or email address',label: 'Username or email address',type: 'text'},
    ...
    const showResetPassword = e => {
        e.preventDefault();
        document.querySelector('.' + loginformc.RessetPassword).classList.toggle(loginformc.Show);
    }

    const onResetPasswordChangeValue = (e) => setResetPassword({ ...resetPassword, [e.target.name]: e.target.value });
    ...
    return (
        ...
        <React.Fragment>
            {!login.errors ? <Spinner /> :
            <React.Fragment>
                {login.successMessage ? <ResponseMessage message={login.successMessage} /> : null}
                {resetPassword.successMessage ? <ResponseMessage message={resetPassword.successMessage} /> : null}
                {resetPassword.failedMessage ? <ResponseMessage ClassName="Danger" message={resetPassword.failedMessage} /> : null}
                {login.failedMessage ? <ResponseMessage ClassName="Danger" message={login.failedMessage} /> : null}
                ...
                <form onSubmit={onLoginFormSubmit}>
                    ...
                    <div className={classes.formBox + ' ' + classes.Checkbox}>
                        <input type="checkbox" name="rememberMe" checked={login.rememberMe} onChange={onChangeLoginValue}/>
                        <label>Remember me</label>
                    </div>
                    <div className={classes.formBox}>
                        <button type="submit" className="login">Login</button>
                        <a href="/" title="" onClick={showResetPassword}>Lost your password?</a>
                    </div>
                </form>
                <form className={loginformc.RessetPassword} onSubmit={onResetPasswordSubmit}>
                    <LoginRegisterInputs
                        name="email"
