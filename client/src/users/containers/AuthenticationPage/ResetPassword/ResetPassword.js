...
const ResetPassword = props => {
	...
	useEffect(() => {
		...
		SetState({ ...props.resetState });
	}, []);

	useEffect(() => {
		SetState({ ...State, ...props.resetState });
		checkLocalStorageToken();
	}, [props.resetState]);
	...
	return (
		<React.Fragment>
			{!State.errors ? (
				...
				<React.Fragment>
					{token === false ? (
						<Redirect to={props.resetState.redirectTo} />
					) : (
						<React.Fragment>
							{State.errors.expiredToken ? (
								<ResponseMessages
									ClassName="Danger"
									message={State.errors.expiredToken}
								/>
							) : null}
							{State.failedMessage ? (
								<ResponseMessages
							...
							{State.successMessage ? (
								<ResponseMessages message={State.successMessage} />
							) : null}
