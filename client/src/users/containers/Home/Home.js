...
const Home = props => {
	...
	useEffect(() => {
		if (props.product.ourProducts.length > 0) {
			const newTopColumn = props.product.ourProducts
				.slice(0, Math.floor(props.product.ourProducts.length / 2))
				.map(product => product);
			const newBottomColumn = props.product.ourProducts
				.slice(Math.floor(props.product.ourProducts.length / 2))
				.map(product => product);
	...
	return (
		<React.Fragment>
			...
			{props.product.weeklyOffer.length > 0 && (
				<Offer
					label="Weekly Offer"
					products={props.product.weeklyOffer}
					weekly={true}
				/>
			)}
			...
			{props.product.dailyOffer.length > 0 && (
				<Offer label="Daily Offer" products={props.product.dailyOffer} />
			)}
						...
						<div className="row ">
							<OwlCarousel
								...
								{topColumn.map((product, index) => {
									...
									const diffBottom = calculateDateDifferenece(
										bottomColumn[index].createdAt
									);
									return (
										<div className="owl-carousel-item" key={index}>
												...
												<div className="imagebox style1">
													<div className="box-image">
														<Link to={`/product?id=${bottomColumn[index]._id}`}>
															<img
																src={
																	bottomColumn[index].options[0].featuredPicture
																}
																alt=""
															/>
														</Link>
													...
													<div className="box-content">
														...
														<div className="product-name">
															<Link
																to={`/product?id=${bottomColumn[index]._id}`}>
																{bottomColumn[index].name}
															</Link>
														</div>
														<div className="price">
															<span className="regular">
																From: ${bottomColumn[index].price}
															</span>
														</div>
													</div>
												</div>
