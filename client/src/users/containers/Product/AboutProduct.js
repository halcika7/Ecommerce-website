...
const AboutProduct = ({
	...
	const date = dailyOffer
		? new Date(dailyOffer.expires).toDateString()
		: weeklyOffer
		? new Date(weeklyOffer.expires).toDateString()
		: null;
	return (
		<React.Fragment>
			<h4>{name}</h4>
			...
			{dailyOffer && (
				<p>
					Product is on Daily discount and every product option is discounted by{' '}
					{dailyOffer.discount}%. This offer expires on {date}.
				</p>
			)}
			{weeklyOffer && (
				<p>
					Product is on Daily discount and every product option is discounted by{' '}
					{weeklyOffer.discount}%. This offer expires on {date}.
				</p>
			)}
			<p className="sku">
			...
			<p className="new-price mt-2">
				...
				{discount !== 0 &&
					`Price with Discount: $${price +
						aditionalPrice -
						(price + aditionalPrice) / discount} `}
			</p>
			{(subCat === 'Headphones' || subCat === 'Speakers') && (
				<p>
					Wir: <strong>{wifi.toString()}</strong>
				</p>
			)}
			{(subCat === 'Headphones' || subCat === 'Speakers') && (
				<p>
					Bluetooth: <strong>{bluetooth.toString()}</strong>
				</p>
			)}
		</React.Fragment>
	);
