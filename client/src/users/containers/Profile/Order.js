...
const Order = props => {
    ...
    return (
        <div className={classes.wrapper}>
            <div className={classes.modal} >
                <div className='container'>
                    <div className='row'>
                        ...
                        {(props.order.coupon && props.order.coupon.applied) && <div className={classes.col + ' col-12 col-sm-4 col-md-3 mb-3'}>
                        ...
                        {props.order.products && props.order.products.map((product, index) =>
                            <div className={classes.flexProduct} key={index}>
                                ...
                                <div className='form-group ml-3'>
                                    ...
                                    <label className='d-block'>Quantity: {product.quantity}</label>
                                    <div className='d-flex flex-column'>
                                        {Object.keys(product.options).map((option, index) => 
                                            <label className={classes.capitalize} key={index}>{option}: {product.options[option]}</label>    
                                        )}
                                    </div>
                                </div>
                            </div>    
            ...
            <div className={classes.backdrop} onClick={props.setShowOrder}></div>
        </div>
    );
}
