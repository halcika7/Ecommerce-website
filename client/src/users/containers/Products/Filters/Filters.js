...
const Filters = ({ values, choosenValues, setChoosenValues, filterChange, property, label }) => {
    ...
    return (
        <div className="col-12">
            <p
                className={show ? 'dropdown-category-links color' : 'dropdown-category-links down'}
                onClick={clicked}>
                {label}
                ...
                <div className="category-search-list">
                    <ul className={classes.checkbox + ' checkbox'}>
                        {values.map((values, index) => {
                            const findIndex = choosenValues.findIndex(
                                val => val == values[property]
                            );
                            const bool = label === 'Bluetooth' || label === 'Wifi' ? true : false;
                            const someNumber = new Date().getTime() * Math.random();
                            return (
                                <li
                                    className={classes.inputGroup + ' input-group'}
                                    key={index + someNumber}>
                                    <input
                                        type="checkbox"
                                        name={
                                            label === 'Bluetooth' || label === 'Wifi'
                                        ...
                                        onChange={e =>
                                            filterChange(e, choosenValues, setChoosenValues, bool)
                                        }
                                    />
                                    <label htmlFor={'i' + someNumber}>
                                        {label === 'Bluetooth' || label === 'Wifi'
                                            ? values[property].toString()
                                            : values[property]}
                                        {property === 'graphic' ||
