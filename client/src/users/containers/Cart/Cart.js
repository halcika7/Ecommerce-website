...
const Cart = props => {
	...
	const removeFromCart = (e, sku) => {
		e.preventDefault();
		props.removeFromCart(sku);
	};
	const deleteFromSavedForLater = (e, sku) => {
		e.preventDefault();
		props.deleteFromSavedForLater(sku);
	};
	...
	const updateCartItem = (e, sku) => {
		e.preventDefault();
		props.updateCartItem(sku, e.target.value);
	};
	const moveToSaveForLater = (e, sku) => {
		e.preventDefault();
		props.moveToSaveForLater(sku);
	};
	const moveToCart = (e, sku) => {
		e.preventDefault();
		props.moveToCart(sku);
	};
				...
				<div className={c.row + ' row'}>
					<div className={c.collg8 + ' col-lg-8'}>
										...
										<tr key={index}>
											<td>
												<div className={c.productname}>
													...
													<div
														className={
															props.productErrors.length > 0 &&
															props.productErrors[index].error
																? 'invalid ' + c.name
																: c.name
														}>
											...
											<td>
												<input
													...
													min="1"
													value={item.quantity}
													onChange={e => updateCartItem(e, item.sku)}
												/>
											</td>
												...
												<div className={c.options}>
													{Object.keys(item.options).length > 0 ? (
														Object.keys(item.options).map(option => (
															<p key={option}>
																{option}: {item.options[option].toString()}
															</p>
														))
													) : (
														<p>No options</p>
											...
											<td>
												<h6 className={c.price}>${item.total}</h6>
											</td>
											<td>
												<div className={c.buttons}>
													<a
														...
														onClick={e => moveToSaveForLater(e, item.sku)}
														data-toggle="data-tooltip"
														data-tooltip="Move to Save for Later">
														<i className={c.fasave + ' far fa-save'} />
													</a>
													<a
														href="/"
														onClick={e => removeFromCart(e, item.sku)}
														className="add-to-cart-wishlist"
														data-toggle="data-tooltip"
														data-tooltip="Delete">
														<i className={c.fatrashalt + ' far fa-trash-alt'} />
													</a>
												</div>
											</td>
										</tr>
						...
						{Object.keys(props.cart.coupon).length === 0 && (
							<Coupon setCoupon={setCoupon} applyCoupon={applyCoupon} />
						)}
					</div>
							...
							{Object.keys(props.cart.coupon).length > 0 && (
								<React.Fragment>
									<div className={c.subtotal}>
										<p>Subtotal:</p>
										<p className={c.pricesubtotal}>
											${props.cart.totals.subtotalBefore}
										</p>
									</div>
									<div
										className={
											Object.keys(props.couponErrors).length > 0
												? 'invalid ' + c.couponcode
												: c.couponcode
										}>
										<div>
											...
											<button
												...
												className="btn btn-sm btn-danger"
												data-toggle="data-tooltip"
												data-tooltip="Remove Coupon">
												Remove
											</button>
										</div>
										<p className={c.minusmoney}>
											-
											{props.cart.coupon.type === 'percent'
												? props.cart.coupon.value + '%'
												: '$' + props.cart.coupon.value}
										</p>
									</div>
									{Object.keys(props.couponErrors).length > 0 && (
										<div className="invalid-feedback">{props.couponErrors}</div>
									)}
									<hr />
								</React.Fragment>
							)}
				...
				<div className={c.collg12 + ' col-12 mb-4'}>
					<h4>
						...
						{props.cart.saveForLaterItems.length > 0 && (
							<button
								onClick={destroySaveForLater}
								className="btn btn-sm btn-danger ml-2"
								data-tooltip="Destroy Saved for Later">
								Destroy Saved for Later
							</button>
						)}
					</h4>
					{props.cart.saveForLaterItems.length > 0 ? (
									...
									<tr key={index}>
										...
										<td>
											<div className={c.buttons}>
												<a
													href="/"
													onClick={e => moveToCart(e, item.sku)}
													className="add-to-cart-wishlist"
													data-toggle="data-tooltip"
													data-tooltip="Move to Cart">
													<i className={c.fasave + ' fas fa-shopping-cart'} />
												</a>
												<a
													href="/"
													onClick={e => deleteFromSavedForLater(e, item.sku)}
													className="add-to-cart-wishlist"
													data-toggle="data-tooltip"
													data-tooltip="Delete">
													<i className={c.fatrashalt + ' far fa-trash-alt'} />
