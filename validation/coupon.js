...
exports.validateCoupon = async (data) => {
    ...
    if (!regex.test(data.name)) {
		errors.name = 'Name can contain only numbers and letters.No white space allowed!';
    }
    ...
    if (data.type === 'percent' && (parseInt(data.value) < 1 || parseInt(data.value) > 99)) {
		errors.value = 'Value for choosen type must be greather than 0 and less than 100';
    }
